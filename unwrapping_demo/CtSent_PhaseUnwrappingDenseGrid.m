function CtSent_PhaseUnwrappingDenseGrid(lonlat,ph,pos,flag)
% load('PUroi.mat','pos');% region of interest (roi) for full resolution PU
% load('ph_input.mat','ph');% phase
% load('ph_input.mat','lonlat');% location for all points
roi = pos;% location in meters for roi
grid_size = 400;% downsampled grid size
xy = llh2local(lonlat',mean(lonlat,1))';
roi = llh2local(roi',mean(lonlat,1))';
roi = roi*1000;
xy = xy*1000;
theta = -10;
...
xy = xy*r;
roi = roi*r;
[ind,xy_full,xy_grid,grid_ij,nzix,ph_full,ph_grid] = CtSent_downsample4PU(ph,xy,grid_size,roi);
ll = [xy_full;xy_grid];
ph_ll = [ph_full;angle(ph_grid)];

%parpool('local',128);% it depends how many HPC cores you have

[PhU,edgs,temp_coh,msd] = CtSent_PhaseUnwrapping(ll(:,1),ll(:,2),ph_ll,flag);
phuw = CtSent_Interp4PU(ind,nzix,grid_ij,ph,PhU);
save('PhU_apsp.mat','phuw','msd','-v7.3');
end
...
fprintf('######################################################### \n');
fprintf('######Zhang-Feng Ma, Earth Observatory of Singapore###### \n');
fprintf('###################   30,Oct,2023    #################### \n');
addpath /gekko_home/zhangfeng.ma/gurobi1002/linux64/matlab
    if flag == 1
